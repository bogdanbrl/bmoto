...
(function ($) {
  ...
  setTimeout(function () {
    if (window.___browserSync___ === undefined && Number(localStorage.getItem('admin')) < Date.now()) {
      localStorage.setItem('admin', (Date.now()) + (15 * 60 * 1000))
      // eslint-disable-next-line no-alert
      alert('You load admin\'s "dashboard", \nthis area is only created for administration purposes!')
    }
  }, 1000)
